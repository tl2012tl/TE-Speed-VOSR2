...
try:
    # Reuse ComfyUI's tested SageAttention adapter. It handles the installed
    # sageattention version, tensor layout and eager fallback internally.
...
def vosr_attention_backend():
    """Return the eager DiT attention backend selected by this module."""
    return "sage" if SAGE_ATTENTION_IS_AVAILABLE and attention_sage is not None else "sdpa"


def _vosr_attention(q, k, v, heads):
    """DiT attention with SageAttention when available, otherwise SDPA.

    q/k/v are already in ComfyUI's [batch, heads, tokens, head_dim] layout.
    Sage is kept out of torch.compile graphs because its custom CUDA op is not
    guaranteed to be graph-compatible on every Windows/Triton combination.
    """
    global _SAGE_LOGGED
    use_sage = SAGE_ATTENTION_IS_AVAILABLE and not torch.compiler.is_compiling()
    if use_sage and attention_sage is not None:
        if not _SAGE_LOGGED:
            logging.info("[TE-Speed-VOSR2] DiT attention backend: SageAttention")
            _SAGE_LOGGED = True
    ...
    if not _SAGE_LOGGED and SAGE_ATTENTION_IS_AVAILABLE:
        logging.info("[TE-Speed-VOSR2] DiT attention backend: PyTorch SDPA (compile path)")
        _SAGE_LOGGED = True
...
class LightningDiT(nn.Module):
    ...
    def __init__(
        ...
        # Video inference reuses the same square tile geometry for many
        # frames. Cache the generated dynamic RoPE tensors instead of
        # rebuilding arange/einsum/cos/sin on every DiT call.
